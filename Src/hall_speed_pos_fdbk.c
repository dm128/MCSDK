...
__weak void *HALL_TIMx_CC_IRQHandler(void *pHandleVoid)
{
  ...
  if (pHandle->SensorIsReliable)
  {
    ...
    if ( HT.reliable == false )
    {
      h3 = LL_GPIO_IsInputPinSet( pHandle->H3Port, pHandle->H3Pin );
      h2 = LL_GPIO_IsInputPinSet( pHandle->H2Port, pHandle->H2Pin );
      h1 = LL_GPIO_IsInputPinSet( pHandle->H1Port, pHandle->H1Pin );

      if ( HT.hHallFlagCnt > 0 )
      {
        if ( HT.previousH3 != h3 )
        {
          HT.H3Connected = true;
      ...
      pHandle->HallState =(uint8_t) ((h3 << 2) | (h2 << 1) | h1);
    }
    else
    {
    ...
    pHandle->HallState = pHandle->HallState ^ pHandle->PinToComplement;
    ...
    switch (pHandle->HallState)
    {
      case STATE_5:
      {
        if (STATE_4 == bPrevHallState)
        {
		  ...
		  if (HT.sm_state == HT_DETECTING_EDGE)
          {
            HT.shiftEdge_state = SHIFT_EDGE_5;
            HT.sm_state = HT_GET_ANGLE_EDGE;
          }
        }
      ...
      case STATE_1:
      {
        if (STATE_5 == bPrevHallState)
        {
		  ...
		  if (HT.sm_state == HT_DETECTING_EDGE)
          {
            HT.shiftEdge_state = SHIFT_EDGE_1;
            HT.sm_state = HT_GET_ANGLE_EDGE;
          }
		}
      ...
      case STATE_3:
      {
        if (STATE_1 == bPrevHallState)
        {
		  ...
		  if (HT.sm_state == HT_DETECTING_EDGE)
          {
            HT.shiftEdge_state = SHIFT_EDGE_3;
            HT.sm_state = HT_GET_ANGLE_EDGE;
          }
		}
      ...
      case STATE_2:
      {
        if (STATE_3 == bPrevHallState)
        {
		  ...
		  if (HT.sm_state == HT_DETECTING_EDGE)
          {
            HT.shiftEdge_state = SHIFT_EDGE_2;
            HT.sm_state = HT_GET_ANGLE_EDGE;
          }
		}
      ...
      case STATE_6:
      {
        if (STATE_2 == bPrevHallState)
        {
		  ...
		  if (HT.sm_state == HT_DETECTING_EDGE)
          {
            HT.shiftEdge_state = SHIFT_EDGE_6;
            HT.sm_state = HT_GET_ANGLE_EDGE;
          }
		}
      ...
      case STATE_4:
      {
        if (STATE_6 == bPrevHallState)
        {
		  ...
		  if (HT.sm_state == HT_DETECTING_EDGE)
          {
            HT.shiftEdge_state = SHIFT_EDGE_4;
            HT.sm_state = HT_GET_ANGLE_EDGE;
          }
		}
...
static void HALL_Init_Electrical_Angle(HALL_Handle_t *pHandle)
{
  ...
  else
  {
    ...
    switch (pHandle->HallState)
    {
      ...
      default:
      {
      /*In case of motor profiler, SensorIsReliable must always be true */
        pHandle->SensorIsReliable = true;
        break;
      }
    }
