...
{
  return (SPD_GetAvrgMecSpeedUnit(&HALL_M1._Super));
}
...
{
  int16_t returnValue;
  returnValue = (SPD_GetAvrgMecSpeedUnit(&HALL_M1._Super) * U_RPM) / SPEED_UNIT;
...
{
  return (SPD_GetElAngle(&HALL_M1._Super));
}
...
{
  return (((float_t)SPD_GetElAngle(&HALL_M1._Super)) / S16ToRAD);
}
